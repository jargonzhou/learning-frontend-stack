...
export type NavigationGuard<V extends Vue = Vue> = (
  to: Route,
  from: Route,
  // next callback: proceed, cancel, redirect, error
...
export declare class VueRouter {
  ...
  /**
   * Add a navigation guard that executes before any navigation./导航前执行
   *
  ...
  /**
   * Add a navigation hook that is executed after every navigation. Returns a function that removes the registered hook./导航后执行
   *
...
// the route abstraction
export interface Route {
  path: string                                   // route path
  name?: string | null                           // route name
  hash: string
  query: Dictionary<string | (string | null)[]>  // route query param???
  params: Dictionary<string>                     // route path params
  fullPath: string
  matched: RouteRecord[]
